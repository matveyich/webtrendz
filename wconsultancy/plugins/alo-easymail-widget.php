...
// wtz don't want to show smt other for registered
$show_for_registered = false;
	if (is_user_logged_in() and $show_for_registered == true) {
        // For REGISTERED USER
        if ( $subscriber_id ){
       ...
       // $html .= "  <tr>\n";
    	$html .= "
			<form name='alo_easymail_widget_form' id='alo_easymail_widget_form' method='post' action='' onsubmit='alo_em_pubblic_form();return false;'>
				<ul>
					<li>
    	                <label>".__("E-mail", "alo-easymail")."</label>
    	            </li>
    	            <li>
						<input type='hidden' name='alo_em_opt_email' value='Guest' id='opt_email' size='10' maxlength='50' />\n
						<input type='text' class='newsletterTextInput'' name='alo_em_opt_email' value='". $alo_em_opt_email ."' id='opt_email' maxlength='50' />\n
					</li>";
 		$html .= $lists_table; // add lists table
        $html .= "
					<li>
						<input type='submit' class='primaryBtn' name='submit' value='".__("join", "alo-easymail")."' />\n
					</li>
				</ul>
			</form>\n";
