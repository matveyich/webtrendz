...
$wtz_fp_slug = 'featured-portfolio'; //featured portfolio slug
...
function wtz_testimonials_widget(){
	global $wtz_fp_slug;
	$projects = simple_portfolio_get_projects('portfolio-tags',$wtz_fp_slug);// taxonomy slug is taken from wp_terms
...
$text_media = simple_portfolio_media($post->ID, 'text');
?>
								<li>
                                    <div class="quoteholder">
                                        <blockquote>

<?php $text = $text_media[0];?>
<?php echo $text['value']; ?>
...
$clients = simple_portfolio_get_clients();
$client_names = array();
foreach ($clients as $client){
$result .= '
					<label><input type="radio" name="type" value="'.$client->slug.'">'.$client->name.'></label>
';
}
$result .= '
				</fieldset>
...
foreach ($positions as $k=>$position):
	...
	$clients = simple_portfolio_get_clients($post->ID);
	$client_names = array();
foreach ($clients as $client){
	$client_names[] = $client->slug;
}
	$client_names = implode(' ', $client_names);

$result .= '
			<li data-id="id-'.$k.'" data-type="'.$client_names.'">
              <a href="'.$permalink.'">
