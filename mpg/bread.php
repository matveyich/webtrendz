...
function breadcrumbs($display = true,$home_name = false) {
	if ($home_name == false) $home_name = get_bloginfo('name');
	/*
	CONFIGURUATION
	...
	*/
	...
	if ( !empty($cat) ) {
			...
			if ( !stristr($cat,'-') )
				...
				$title = '<a href="' . get_bloginfo('url') . '">' . $home_name . '</a> ' . $sep . ' Category:' . $zmake;
	if (!empty($search))
	{
	...
	elseif ($search != '')
    {
		...
		$title = '<a href="' . get_bloginfo('url') . '">' . $home_name . '</a> ' . $sep . ' Category: ' . $zmake2;
	...
	elseif ($search != '')
    {
		...
		$title = '<a href="' . get_bloginfo('url') . '">' . $home_name . '</a> ' . $sep . ' ' . $title->display_name;
	}
	if ( !empty($author_name) ) {
		// We do a direct query here because we don't cache by nicename.
		$title = '<a href="' . get_bloginfo('url') . '">' . $home_name . '</a> ' . $sep . ' ' . $wpdb->get_var("SELECT display_name FROM $wpdb->users WHERE user_nicename = '$author_name'");
	}	
	 if ( is_single() ) 
	 { 
	 		if ($year == "0" || $monthnum == "0" || $day == "0")
	 		{
				$title = '<a href="' . get_bloginfo('url') . '">' . $home_name . '</a> ' . $sep . ' ' . the_title2(); 
			}
			else
			{
				$title = '<a href="' . get_bloginfo('url') . '">' . $home_name . '</a> ' . $sep . ' <a href="' . get_bloginfo('url') . '/' . $year . '/">' . $year . '</a> ' . $sep . ' <a href="' . get_bloginfo('url') . '/' . $year . '/' . zeroise($monthnum, 2) . '/">' . $wp_locale->get_month($monthnum) . '</a> ' . $sep . ' <a href="' . get_bloginfo('url') . '/' . $year . '/' . zeroise($monthnum, 2) . '/' . zeroise($day, 2) . '">' . $day . '</a> ' . $sep . ' ' . the_title2(); 
			}
	}	
	elseif ( is_page() )
	{
	...
	do
	{
		...
		$title = '<a href="' . get_bloginfo('url') . '">' . $home_name . '</a> ' . $sep . ' ' . $middle_title . the_title2();
	}
	elseif ( !empty($year) ) {
		$title = '<a href="' . get_bloginfo('url') . '">' . $home_name . '</a> ' . $sep . ' <a href="' . get_bloginfo('url') . '/' . $year . '/">' . $year . '</a> ';
		if ( !empty($monthnum) )
	...
	if (!empty($search))
	{
		$title = '<a href="' . get_bloginfo('url') . '">' . $home_name . '</a> ' . $sep . ' Search Results for ' . $search;
	}
	elseif ($search != '')
    {
	$title = '<a href="' . get_bloginfo('url') . '">' . $home_name . '</a> ' . $sep . ' Search Results for ' . $search;
...
function the_title2($before = '<li class="current">', $after = '</a>', $echo = false) {
	$title = get_the_title();
	if ( strlen($title) > 0 ) {
