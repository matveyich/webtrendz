...
function who_is_online_install()
{
  ...
  PRIMARY KEY (ip)
    ...
    dbDelta($sql);
    add_option('who_is_online_version', '0.1.4');
...
if (version_compare(get_option('who_is_online_version'), '0.1.4', '<'))
{
...
function who_is_online_update()
{
  ...
  PRIMARY KEY (ip)
    ...
    require_once (ABSPATH . 'wp-admin/includes/upgrade.php');
    dbDelta($sql);
    update_option('who_is_online_version', '0.1.4');
}
...
{
    wp_add_dashboard_widget('who_is_online_dashboard_widget', __('Who Is Online'), 'who_is_online_dashboard_widget');
}
function who_is_online_dashboard_widget()
{
    ...
    printf(_n('There is currently %d person online.', 'There is currently %d person online.', $total_users), $total_users);
    echo '<br><br>';
    printf(_n('%d member.', '%d members.', $members), $members);
    echo '<br>';
    printf(_n('%d guest.', '%d guests.', $guests), $guests);
    echo '<br><br>';
...
function who_is_online_add_options_page()
{
    ...
    {
        add_options_page('Who Is Online', __('Who Is Online'), 8, __FILE__, 'who_is_online_options_page');
    }
}
function who_is_online_options_page()
{
    if(isset($_POST['minutes']) and !empty($_POST['minutes']))
    {
        ...
        {
            echo '<div id="message" class="updated fade"><p><strong>'.__('You entered an invalid value').'</strong></p></div>';
        }
    }
...
function who_is_online_output_options_page()
{
	...
	<form method="post">
    <h2>'.__('Who\'s Online').'</h2>
    <fieldset class="options" name="general">
      <legend>'.__('General settings').'</legend>
      <table width="100%" cellspacing="2" cellpadding="5" class="editform">
        <tr>
          <th nowrap valign="top" width="33%">'.__('Minutes Before Offline').'</th>
          <td><input type="text" name="minutes" id="minutes" value="' . get_option('who_is_online_time_before_off') . '" /> <span class="description">'.__('Determines how much time will pass before someone is considered offline.').'</span>
          </td>
        </tr>
      </table>
    </fieldset>
...
{
    register_sidebar_widget(__('Who Is Online'), 'who_is_online_widget');
}
...
function who_is_online_add_users_page()
{
    ...
    {
        add_users_page(__('Who Is Online'), __('Who Is Online'), 8, __FILE__, 'who_is_online_display_online_users');
    }
}
...
<h2>'.__('Online Users').'</h2>
<table class="widefat fixed" cellspacing="0">
<thead>
<tr class="thead">
	<th scope="col" id="username" class="manage-column column-username" style="">'.__('Username').'</th>
	<th scope="col" id="name" class="manage-column column-name" style="">'.__('IP').'</th>
	<th scope="col" id="online" class="manage-column column-online" style="">'.__('Time Online').'</th>
	<th scope="col" id="views" class="manage-column column-views" style="">'.__('Page Views').'</th>
	<th scope="col" id="role" class="manage-column column-role" style="">'.__('Viewing').'</th>
</tr>
</thead>
<tfoot>
<tr class="thead">
