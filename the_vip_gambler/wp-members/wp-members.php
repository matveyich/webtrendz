...
function wpmem_securify ($content)
{
  ...
  if (is_single()) {
    ...
    {
      $txt__ = explode('<!--more-->', $content, 2);
//      $txt__ = explode('<span id="more-114"></span>', $content, 2);
      $content = $txt__[0];
	 // WTZ check if there are two items in array, and only then 
      if (count($txt__) != 1) {
      $content .= "<strong><p>&nbsp;</p></strong>";
      $content .= "<strong><p>Here is information only for logged users.</p></strong>";
      $content .= "<strong><p>Please login for more details!</p></strong>";
	  }
	  // WTZ
/*
...
function wpmem_register()
{
  ...
  global $username,$password,$fname,$lname,$addr1,$addr2,$city,
    $thecity,$thestate,$zip,$country,$phone1,$email,$description,
	$gp_casino,$gp_poker,$gp_sports,$fcg_slots,$fcg_blackjack,$fcg_roulette,$fcg_sicbo,$fcg_baccarat,$fcg_videopoker,$fcg_craps,$fcg_keno,$fcg_paigowpoker,$fp_texas,$fp_omaha,$fp_omahahilo,$fp_7card,$fp_headsup,$fp_smg,$fp_tournaments,$ldc,$fop,$cmw,$cmcbp,$fvp;
  ...
  $gp_casino	= $_POST['gp_casino'];
  $gp_poker		= $_POST['gp_poker'];
  $gp_sports	= $_POST['gp_sports'];
  $fcg_slots	= $_POST['fcg_slots'];
  $fcg_blackjack = $_POST['fcg_blackjack'];
  $fcg_roulette	= $_POST['fcg_roulette'];
  $fcg_sicbo	= $_POST['fcg_sicbo'];
  $fcg_baccarat	= $_POST['fcg_baccarat'];
  $fcg_videopoker = $_POST['fcg_videopoker'];
  $fcg_craps	= $_POST['fcg_craps'];
  $fcg_keno		= $_POST['fcg_keno'];
  $fcg_paigowpoker	= $_POST['fcg_paigowpoker'];
  $fp_texas		= $_POST['fp_texas'];
  $fp_omaha		= $_POST['fp_omaha'];
  $fp_omahahilo	= $_POST['fp_omahahilo'];
  $fp_7card		= $_POST['fp_7card'];
  $fp_headsup	= $_POST['fp_headsup'];
  $fp_smg		= $_POST['fp_smg'];
  $fp_tournaments	= $_POST['fp_tournaments'];
  $ldc		= $_POST['ldc'];
  $fop		= $_POST['fop'];
  $cmw		= $_POST['cmw'];
  $cmcbp	= $_POST['cmcbp'];
  ...
  //$description = $_POST['description'];
	...
	{	
		$wpmem_themsg = "email is incorrect";
	}
  ...
  if ( $wpmem_themsg ) {
    ...
    if (username_exists($username)) {
      ...
      //everything checks out, so go ahead and insert
        ...
        update_usermeta( $user_id, 'gp_casino', $gp_casino);
        update_usermeta( $user_id, 'gp_poker', $gp_poker);
        update_usermeta( $user_id, 'gp_sports', $gp_sports);
        update_usermeta( $user_id, 'fcg_slots', $fcg_slots);
        update_usermeta( $user_id, 'fcg_blackjack', $fcg_blackjack);
        update_usermeta( $user_id, 'fcg_roulette', $fcg_roulette);
        update_usermeta( $user_id, 'fcg_sicbo', $fcg_sicbo);
        update_usermeta( $user_id, 'fcg_baccarat', $fcg_baccarat);
        update_usermeta( $user_id, 'fcg_videopoker', $fcg_videopoker);
        update_usermeta( $user_id, 'fcg_craps', $fcg_craps);
        update_usermeta( $user_id, 'fcg_keno', $fcg_keno);
        update_usermeta( $user_id, 'fcg_paigowpoker', $fcg_paigowpoker);
        update_usermeta( $user_id, 'fp_texas', $fp_texas);
        update_usermeta( $user_id, 'fp_omaha', $fp_omaha);
        update_usermeta( $user_id, 'fp_omahahilo', $fp_omahahilo);
        update_usermeta( $user_id, 'fp_7card', $fp_7card);
        update_usermeta( $user_id, 'fp_headsup', $fp_headsup);
        update_usermeta( $user_id, 'fp_smg', $fp_smg);
        update_usermeta( $user_id, 'fp_tournaments', $fp_tournaments);
        update_usermeta( $user_id, 'ldc', $ldc);
        update_usermeta( $user_id, 'fop', $fop);
        update_usermeta( $user_id, 'cmw', $cmw);
        update_usermeta( $user_id, 'cmcbp', $cmcbp);
        update_usermeta( $user_id, 'fvp', $fvp);
...
function wpmem_inc_sidebar()
{
  ...
  */
  ...
  <?php 
  // WTZ FIX START
  $link = get_settings('home') . "/vipconcierge/members-area/"; ?>
  <p>
    <a href="<?php echo $link; ?>?a=registration">Registration</a>
    &nbsp;|&nbsp;
    <a href="<?php echo $link; ?>?a=pwdreset">Forgot password?</a>
  </p>
<?// WTZ FIX END?>
  <?php } else {
  /*
...
function wpmem_inc_registration($fields,$toggle = 'new',$heading = '')
{
  ...
  global $username,$fname,$lname,$addr1,$addr2,
    $city,$thestate,$zip,$country,$phone1,$email,$description,
	$gp_casino,$gp_poker,$gp_sports,$fcg_slots,$fcg_blackjack,$fcg_roulette,$fcg_sicbo,$fcg_baccarat,$fcg_videopoker,$fcg_craps,$fcg_keno,$fcg_paigowpoker,$fp_texas,$fp_omaha,$fp_omahahilo,$fp_7card,$fp_headsup,$fp_smg,$fp_tournaments,$ldc,$fop,$cmw,$cmcbp,$fvp;
  ...
  if (is_user_logged_in()) {
	...
	$gp_casino	= get_usermeta( $user_ID, 'gp_casino');
    $gp_poker   = get_usermeta( $user_ID, 'gp_poker');
    $gp_sports   = get_usermeta( $user_ID, 'gp_sports');
    $fcg_slots   = get_usermeta( $user_ID, 'fcg_slots');
    $fcg_blackjack   = get_usermeta( $user_ID, 'fcg_blackjack');
    $fcg_roulette   = get_usermeta( $user_ID, 'fcg_roulette');
    $fcg_sicbo   = get_usermeta( $user_ID, 'fcg_sicbo');
    $fcg_baccarat   = get_usermeta( $user_ID, 'fcg_baccarat');
    $fcg_videopoker  = get_usermeta( $user_ID, 'fcg_videopoker');
    $fcg_craps    = get_usermeta( $user_ID, 'fcg_craps');
    $fcg_keno    = get_usermeta( $user_ID, 'fcg_keno');
    $fcg_paigowpoker    = get_usermeta( $user_ID, 'fcg_paigowpoker');
    $fp_texas    = get_usermeta( $user_ID, 'fp_texas');
    $fp_omaha    = get_usermeta( $user_ID, 'fp_omaha');
    $fp_omahahilo   = get_usermeta( $user_ID, 'fp_omahahilo');
    $fp_7card    = get_usermeta( $user_ID, 'fp_7card');
    $fp_headsup    = get_usermeta( $user_ID, 'fp_headsup');
    $fp_smg    = get_usermeta( $user_ID, 'fp_smg');
    $fp_tournaments    = get_usermeta( $user_ID, 'fp_tournaments');
    $ldc    = get_usermeta( $user_ID, 'ldc');
    $fop    = get_usermeta( $user_ID, 'fop');
    $cmw    = get_usermeta( $user_ID, 'cmw');
    $cmcbp    = get_usermeta( $user_ID, 'cmcbp');
    $fvp    = get_usermeta( $user_ID, 'fvp');
  ...
  */
  ...
  <form name="form2" method="post" action="<?php the_permalink();//wpmem_chk_qstr();?>">
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="4" class="registrationForm">
    <tr align="left">
    ...
    <tr>
      ...
      <td width="70%" align="left"><?php echo $userdata->user_login?></td>
    </tr>
    <?php } else { ?>
    <tr>
      <td width="30%" align="right">Choose a Username <font color="red">*</font></td>
      <td width="70%"><input name="log" type="text" value="<?php echo $username;?>" /></td>
    </tr>
    <?php } ?>
...
<?}?>    
    ...
    <tr>
      <td colspan="2">Please take a quick moment of your time to complete the following questionnaire and submit to us. One of our VIP Representatives will contact you shortly to find you the best deals at the greatest gaming sites.</td>
    </tr> 
       
	<tr>
      <td align="right">Gaming Preference</td>
      <td>
		<span><input id="gp_casino" type="checkbox" name="gp_casino" value="1" <? if(1==$gp_casino) echo "checked";?>> <label for="gp_casino">Casino</label></span>
		<span><input id="gp_poker" type="checkbox" name="gp_poker" value="1" <? if(1==$gp_poker) echo "checked";?>> <label for="gp_poker">Poker</label></span>
		<span><input id="gp_sports" type="checkbox" name="gp_sports" value="1" <? if(1==$gp_sports) echo "checked";?>> <label for="gp_sports">Sports</label></span>
	  </td>
    </tr>
	<tr>
      <td align="right">Favourite Casino Games</td>
      <td>
		<span><input id="gp_slots" type="checkbox" name="fcg_slots" value="1" <? if(1==$fcg_slots) echo "checked";?>> <label for="gp_slots">Slots</label></span>
		<span><input id="fcg_blackjack" type="checkbox" name="fcg_blackjack" value="1" <? if(1==$fcg_blackjack) echo "checked";?>> <label for="fcg_blackjack">Blackjack</label></span>
		<span><input id="fcg_roulette" type="checkbox" name="fcg_roulette" value="1" <? if(1==$fcg_roulette) echo "checked";?>> <label for="fcg_roulette">Roulette</label><br /></span>
		<span><input id="fcg_sicbo" type="checkbox" name="fcg_sicbo" value="1" <? if(1==$fcg_sicbo) echo "checked";?>> <label for="fcg_sicbo">Sic Bo</label></span>
		<span><input id="fcg_baccarat" type="checkbox" name="fcg_baccarat" value="1" <? if(1==$fcg_baccarat) echo "checked";?>> <label for="fcg_baccarat">Baccarat</label></span>
		<span><input id="fcg_videopoker" type="checkbox" name="fcg_videopoker" value="1" <? if(1==$fcg_videopoker) echo "checked";?>> <label for="fcg_videopoker">Video Poker</label></span><br />
		<span><input id="fcg_craps" type="checkbox" name="fcg_craps" value="1" <? if(1==$fcg_craps) echo "checked";?>> <label for="fcg_craps">Craps</label></span>
		<span><input id="fcg_keno" type="checkbox" name="fcg_keno" value="1" <? if(1==$fcg_keno) echo "checked";?>> <label for="fcg_keno">Keno</label></span>
		<span><input id="fcg_paigowpoker" type="checkbox" name="fcg_paigowpoker" value="1" <? if(1==$fcg_paigowpoker) echo "checked";?>> <label for="fcg_paigowpoker">Pai Gow Poker</label></span>
	  </td>
    </tr>
	<tr>
      <td align="right">Favourite Poker</td>
      <td>
		<span><input id="fp_texas" type="checkbox" name="fp_texas" <? if(1==$fp_texas) echo "checked";?> value="1"> <label for="fp_texas">Texas Hold&#8242;em</label></span>
		<span><input id="fp_omaha" type="checkbox" name="fp_omaha" <? if(1==$fp_omaha) echo "checked";?> value="1"> <label for="fp_omaha">Omaha</label></span>
		<span><input id="fp_omahahilo" type="checkbox" name="fp_omahahilo" <? if(1==$fp_omahahilo) echo "checked";?> value="1"> <label for="fp_omahahilo">Omaha Hi-Lo</label></span><br>
		<span><input id="fp_7card" type="checkbox" name="fp_7card" <? if(1==$fp_7card) echo "checked";?> value="1"> <label for="fp_7card">7 Card Stud</label></span>
		<span><input id="fp_headsup" type="checkbox" name="fp_headsup" <? if(1==$fp_headsup) echo "checked";?> value="1"> <label for="fp_headsup">Heads up Poker</label></span>
		<span><input id="fp_smg" type="checkbox" name="fp_smg" <? if(1==$fp_smg) echo "checked";?> value="1"> <label for="fp_smg">SNG&#8242;s</label></span><br>
		<span><input id="fp_tournaments" type="checkbox" name="fp_tournaments" <? if(1==$fp_tournaments) echo "checked";?> value="1"> <label for="fp_tournaments">Tournaments</label></span>
	  </td>
    </tr>
	<tr>
      <td align="right">Do You Play Live Dealer Casino?</td>
      <td>
		<span><input id="liveDealer-yes" type="radio" name="ldc" <? if(1==$ldc) echo "checked";?> value="1"> <label for="liveDealer-yes">Yes</label></span>
		<span><input id="liveDealer-no" type="radio" name="ldc" <? if(0==$ldc) echo "checked";?> value="0"> <label for="liveDealer-no">No</label></span>
	  </td>
    </tr>
	<tr>
      <td align="right">Frequency Of Play</td>
      <td>
		<span><input id="freq-daily" type="radio" name="fop" <? if(1==$fop) echo "checked";?> value="1"> <label for="freq-daily">Daily</label></span>
		<span><input id="freq-weekly" type="radio" name="fop" <? if(2==$fop) echo "checked";?> value="2"> <label for="freq-weekly">Weekly</label></span>
		<span><input id="freq-fortnightly" type="radio" name="fop" <? if(3==$fop) echo "checked";?> value="3"> <label for="freq-fortnightly">Fortnightly</label></span><br />
		<span><input id="freq-monthly" type="radio" name="fop" <? if(4==$fop) echo "checked";?> value="4"> <label for="freq-monthly">Monthly</label></span>
		<span><input id="freq-other" type="radio" name="fop" <? if(5==$fop) echo "checked";?> value="5"> <label for="freq-other">Other</label></span>
	  </td>
    </tr>
	<tr>
      <td align="right">Current Monthly Wager</td>
      <td>
		<span><input id="monthlyWager-1000" type="radio" name="cmw" <? if(1==$cmw) echo "checked";?> value="1"> <label for="monthlyWager-1000">&#163;0-5,000</label></span>
		<span><input id="monthlyWager-1000-5000" type="radio" name="cmw" <? if(2==$cmw) echo "checked";?> value="2"> <label for="monthlyWager-1000-5000">&#163;5,000-50,000</label></span>
		<span><input id="monthlyWager-5000-10000" type="radio" name="cmw" <? if(3==$cmw) echo "checked";?> value="3"> <label for="monthlyWager-5000-10000">&#163;50,000-100,000</label></span><br />
		<span><input id="monthlyWager-1000more" type="radio" name="cmw" <? if(4==$cmw) echo "checked";?> value="4"> <label for="monthlyWager-1000more">&#163;100,000+</label></span>
	  </td>
    </tr>
	<tr>
      <td align="right">Current Monthly Cash Back Programme</td>
      <td>
		<span><input id="cashBack-5" type="radio" name="cmcbp"  <? if(1==$cmcbp) echo "checked";?> value="1"> <label for="cashBack-5">Up to 10%</label></span>
		<span><input id="cashBack-10" type="radio" name="cmcbp"  <? if(2==$cmcbp) echo "checked";?> value="2"> <label for="cashBack-10">10-15%</label></span>
		<span><input id="cashBack-15" type="radio" name="cmcbp"  <? if(3==$cmcbp) echo "checked";?> value="3"> <label for="cashBack-15">15%+</label></span>
	  </td>
    </tr>
	<tr>
      <td align="right">Favourite VIP Reward</td>
      <td>
		<span><input id="vipreward-cash" type="radio" name="fvp" <? if(1==$fvp) echo "checked";?> value="1">  <label for="fcg_keno">Cash Bonus</label></span>
		<span><input id="vipreward-event" type="radio" name="fvp" <? if(2==$fvp) echo "checked";?>  value="2">  <label for="fcg_keno">Hospitality Event</label></span>
		<span><input id="vipreward-gift" type="radio" name="fvp" <? if(3==$fvp) echo "checked";?> value="3">  <label for="fcg_keno">Luxury Gift</label></span>
	  </td>
    </tr>
    ...
    <tr>
      ...
      <td><textarea cols="15" rows="5" name="description"><?php echo $description;?></textarea></td>
    </tr>
...
*/	  
  ...
  ?>
  <h3>Members Additional Fields</h3>
...
if ($show_addr == true) {?>  
    ...
    <tr>
      <td >State (or Province) <font color="red">*</font></td>
      <td><?php echo $thestate;?></td>
    </tr>
    <tr>
      <td >Zip (or Postal Code) <font color="red">*</font></td>
      <td><?php echo $zip;?></td>
    </tr>
    <tr>
...
<?}?>
    ...
    <tr>
      <td >Day Phone <font color="red">*</font></td>
      <td><?php echo $phone1;?></td>
    </tr>
    
	<tr>
      <td >Gaming Preference</td>
      <td>
		<? if(1==$gp_casino) {?><span><?echo "Casino";?></span><br /><?}?>
		<? if(1==$gp_poker) {?><span><?echo "Poker";?></span><br /><?}?>
		<? if(1==$gp_sports) {?><span><?echo "Sports";?></span><br /><?}?>
	  </td>
    </tr>
	<tr>
      <td >Favourite Casino Games</td>
      <td>
		<? if(1==$fcg_slots) {?><span><?echo "Slots";?></span><br /><?}?>
		<? if(1==$fcg_blackjack) {?><span><?echo "Blackjack";?></span><br /><?}?>
		<? if(1==$fcg_roulette) {?><span><?echo "Roulette";?></span><br /><?}?>
		<? if(1==$fcg_sicbo) {?><span><?echo "Sic Bo";?></span><br /><?}?>
		<? if(1==$fcg_baccarat) {?><span><?echo "Baccarat";?></span><br /><?}?>
		<? if(1==$fcg_videopoker) {?><span><?echo "Video Poker";?></span><br /><?}?>
		<? if(1==$fcg_craps) {?><span><?echo "Craps";?></span><br /><?}?>
		<? if(1==$fcg_keno) {?><span><?echo "Keno";?></span><br /><?}?>
		<? if(1==$fcg_paigowpoker) {?><span><?echo "Pai Gow Poker";?></span><br /><?}?>
	  </td>
    </tr>
	<tr>
      <td >Favourite Poker</td>
      <td>
		<? if(1==$fp_texas) {?><span><?echo "Texas Hold&#8242;em";?></span><br><?}?>
		<? if(1==$fp_omaha) {?><span><?echo "Omaha";?></span><br><?}?>
		<? if(1==$fp_omahahilo) {?><span><?echo "Omaha Hi-Lo";?></span><br><?}?>
		<? if(1==$fp_7card) {?><span><?echo "7 Card Stud";?></span><br><?}?>
		<? if(1==$fp_headsup) {?><span><?echo "Heads up Poker";?></span><br><?}?>
		<? if(1==$fp_smg) {?><span><?echo "SNG&#8242;s";?></span><br><?}?>
		<? if(1==$fp_tournaments) {?><span><?echo "Tournaments";?></span><br><?}?>
	  </td>
    </tr>
	<tr>
      <td >Do You Play Live Dealer Casino?</td>
      <td>
		<span><? if(1==$ldc) echo "Yes"; else echo "No";?></span>
	  </td>
    </tr>
	<tr>
      <td >Frequency Of Play</td>
      <td>
		<span><?
		switch($fop)
		{
	...
	<tr>
      <td >Current Monthly Cash Back Programme</td>
      <td>
		<span>
		<?
		switch($cmcbp)
