...
$err = $wpdb->print_error();
    ...
    // update post
    wp_update_post($page);
update_post_meta($page['ID'], '_wp_page_template', 'review_page.php');
    ?>
            ...
            <div id="poststuff" style="min-width:550px">
              <div id="<?php echo user_can_richedit() ? 'postdivrich' : 'postdiv'; ?>" class="postarea">
                <div class="postbox">
                  ...
                  <div class="inside">
                    <?php the_editor_wtz(str_replace("\'", "'", str_replace('\&quot;', '"', stripslashes($data_mdescr))),'pgr_mdescr','pgr_mdescr'); ?>
                  </div>
                </div>
              </div>
            </div>
  ...
  //delete multiboxes
    ...
    if(($pgract == __('Ledit', 'pg-review')) || (($pgract == __('Update MultiBox', 'pg-review'))))
    {
	  ...
	  $pgr_position = $pgrs->position;
	  $pgr_shdescr = $pgrs->shdescr;
	  $pgr_bonustype = $pgrs->bonus_type;
    }
    ...
    <form name="mainform" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>?page=<?php echo plugin_basename(__FILE__); ?>">
      ...
      <div class="wrap">
		...
		<table class="form-table">
		  ...
		  <tr>
            <th width="20%" scope="row" valign="top">Position</th>
            <td width="80%"><input type="text" name="pgr_position" value="<?php echo $pgr_position;?>" /></td>
          </tr>
		  <tr>
			   ...
			   <div id="poststuff">
                <div id="<?php echo user_can_richedit() ? 'postdivrich' : 'postdiv'; ?>" class="postarea">
                  <div class="postbox">
                    ...
                    <div class="inside">
                      <?php #the_editor_wtz(stripslashes($pgr_fullhtml)); ?>
                      <?php the_editor_wtz(str_replace("\'", "'", str_replace('&quot;', '"', stripslashes($pgr_fullhtml))),'pgr_fullhtml','pgr_fullhtml'); ?>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <tr>
            ...
            <td width="80%">
              <select name="pgr_type">
				...
				<option value="Live-dealer" <?php if($pgr_type=='Live-dealer') echo"selected" ?> >Live dealer</option>
              </select>
            </td>
          </tr>
          <tr>
              ...
              <div id="poststuff">
                <div id="<?php echo user_can_richedit() ? 'postdivrich' : 'postdiv'; ?>" class="postarea">
                  <div class="postbox">
                    <h3 class="hndle"><span><?php _e('Description', 'pg-review') ?></span></h3>
                    <div class="inside">
                      <?php the_editor_wtz(str_replace("\'", "'", str_replace('&quot;', '"', stripslashes($pgr_descr))),'pgr_descr','pgr_descr'); ?>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <table class="form-table">
		  ...
		  <tr>
            <th width="20%" scope="row" valign="top"><?php _e('Bonus type', 'pg-review') ?></th>
            <td width="80%">
              <select name="pgr_bonustype">
                <option disabled selected value="">Bonus type</option>
                <option value="1" <?php if($pgr_bonustype==1) echo"selected" ?> >[from value] UPTO [to value]</option>
                <option value="2" <?php if($pgr_bonustype==2) echo"selected" ?> >DEPOSIT [from value] PLAY WITH [to value]</option>
                <option value="3" <?php if($pgr_bonustype==3) echo"selected" ?> >[from value] FREE</option>
              </select>
            </td>
          </tr>
          <tr>
