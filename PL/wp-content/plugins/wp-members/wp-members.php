...
function wpmem()
{
	if (file_exists('captcha/captcha.function.php5'))//including captcha form
	{
		session_start();
		include('captcha/captcha.function.php5');
		//echo 'captcha included';
	}
...
function wpmem_securify ($content) 
{
	...
	if ($wpmem_regchk == "loginfailed" and (!(is_page('upload') or is_page('members-area')))) {
	...
	if ( $wpmem_themsg ) {
		...
		if (username_exists($username)) {
			...
			//everything checks out, so go ahead and insert
				...
				*/
				update_usermeta( $user_id, 'contactme', $contactme);
...
{ 
    ...
    <form name="form1" method="post" action="<?php the_permalink();echo "?a=".$_GET['a']; ?>">
	 <div class="loginArea">
...
?>	</div>
     	<ul>
        	<li>
                ...
                <span><input type="text" name="log" id="username" /></span>
            </li>
            <li>
            	<label for="password">Password</label>
                <span><input type="password" name="pwd" id="password" /></span>
            </li>
             <li>
            	<label for="rememberme">Remember me</label>
                <span><input name="rememberme" type="checkbox" id="rememberme" value="forever" /></span>
            </li>
            
            <input type="hidden" name="redirect_to" value="<?php echo get_permalink(get_page_by_path('supportfaq/forum')) ?>" />
        ...
        <li>Forgot password? <a href="<?php echo $link; ?>a=pwdreset">Click here to reset</a></li>
        </ul>
        <?php } ?>
     </div>
...
function wpmem_inc_sidebar()
{
	...
	*/
    ...
    <form name="form" method="post" action="<?php echo $post_to; ?>">
		<li>
			<span>Username</span>
			<span><input type="text" name="log" /></span>
...
?>		</div>
    ...
    <ul>
         ...
         <?php //if($page == "members"){ 
	  	//$link = wpmem_chk_qstr(); 
	  	$link = get_permalink(get_page_by_path('members-area'));
	  	// this is only shown on the "members" page ?>
...
function wpmem_inc_registration($fields,$toggle = 'new',$heading = '')
{
	...
	if (!$heading) { $heading = "<h3>New Users Registration</h3>"; }
	...
	if (is_user_logged_in()) {
		...
		if ( ($toggle == 'edit') && ($wpmem_regchk != 'updaterr')) {
			...
			$contactme    = get_usermeta($user_ID,'contactme');
	...
	*/
	...
	<form name="form2" method="post" action="<?php the_permalink();/*wpmem_chk_qstr();*/?>">
       ...
       <?php echo $heading; ?>   
        <p>All fields marked with an <span class="required">*</span> are required fields for registration.</p>  
	  ...
	  <ul class="regForm">
		...
		<?php } else { ?>
		<li> 
		  <label for="log">Choose a Username <span class="required">*</span></label>
		  <input id="log" name="log" type="text" value="<?php echo $username;?>" />
		</li>
		<?php } ?>
		...
		<?/*?>
		<li> 
		  <label for="addr1">Address <span class="required">*</span></label>
		  <input name="addr1" id="addr1" type="text" value="<?php echo $addr1;?>" />
		</li>
		<li> 
		  <label for="addr2">Address 2</label>
		  <input name="addr2" id="addr2" type="text" value="<?php echo $addr2;?>" />
		</li>
		<li> 
		...
		<li> 
		  <label for="thestate">State (or Province) <span class="required">*</span></label>
		  <input name="thestate" id="thestate" type="text" value="<?php echo $thestate;?>" />
		</li>
		<li> 
		  <label for="zip">Zip (or Postal Code) <span class="required">*</span></label>
		  <input name="zip" id="zip" type="text" value="<?php echo $zip;?>" />
		</li>
		<li> 
		  <label for="country">Country <span class="required">*</span></label>
		  <input name="country" id="country" type="text" value="<?php echo $country;?>" />
		</li>
		
		<li> 
		  <label for="phone1">Day Phone <span class="required">*</span></label>
		  <input name="phone1" id="phone1" type="text" value="<?php echo $phone1;?>" />
		</li>
		...
		<li> 
		  <label for="contactme">Please contact me to let me know of future promotions</label>
		  <input name="contactme" id="contactme" type="checkbox" value="1" <?php if ($contactme == "1") echo "checked";?> />
		</li>
...
<?
		  ...
		  if(function_exists('captcha_show_image')) {
?>
<img src="<?bloginfo('url');?>/captcha/captcha.image.php5?nocache=<?php echo md5(time()); ?>" border="0"><br>
<input name="magicword" type="text" tabindex="4"><br>
<?			
		  }
?>		
		</li>
...
?>
		<li class="actions"> 
		  ...
		  <?php if ($toggle == 'edit') { ?>
		  	<input class="fltRht" name="a" type="hidden" value="update" />
		  <?php } else {
		  ?>
		    <input class="fltRht" name="a" type="hidden" value="register" />
		  <?php } ?>
			<input class="fltRht" name="redirect_to" type="hidden" value="<?php echo get_permalink(get_page_by_path('supportfaq/forum')) ?>" />
			<input name="Submit" type="submit" value="Submit Details" class="submitBtn" /> 
			&nbsp;&nbsp; 
			<input name="Reset" type="reset" value="Clear Form" class="clearBtn" />
	  ...
	  <table width="400" border="0" cellspacing="0" cellpadding="4">
		...
		<tr> 
		  ...
		  <td width="166">
			...
			<input type="submit" name="Submit" value="Reset Password" class="submitBtn" />
		  </td>
		</tr>
	  </table> 
	  </form> 
...
{ 
	...
	*/
	...
	case ("user"):
		...
		<div class="wpmem_msg errorMsg" align="center">
	...
	case ("captcha"):	
		...
		<div class="wpmem_msg errorMsg" align="center">
			...
			<p><b>Sorry, you have written wrong text on the image</b></p>
	...
	case ("success"):
		...
		<div class="wpmem_msg successMsg" align="center">
...
function wpmem_admin_fields()
{
	...
	*/
		// for contactme custom field
		$wpmem_customfield = 'contactme';?>    
        <tr>
        	<th><label>Subscription (contact me field)</label></th>
        	<td><input id="<?php echo $wpmem_customfield; ?>" type="checkbox" class="input" name="<?php echo $wpmem_customfield; ?>" value="1" <?php if(get_usermeta($user_id, $wpmem_customfield) == "1") echo "checked";?> /></td>
        </tr>
...
function wpmem_admin_update()
{
	...
	$field = 'contactme';
	if($_POST[$field]) update_usermeta($user_id,$field,$_POST[$field]); else update_usermeta($user_id,$field,'0');
}
