...
function pr_edit_subform($field,$id = NULL,$db_table_name = NULL,$field_name = NULL,$field_value = NULL,$number_of_fields = 1,$field_values = NULL,$labels = NULL)
{
	...
	<input type="submit" name="submit" value="<?echo $field_name;?>" class="button-primary" id="<?echo $id?>">
		<?} else {?>
	<input type="submit" name="submit" value="<?echo $field_name;?>" class="submit" id="<?echo $id?>">
		<?}
	break;
	case "checkbox":
		if($id == NULL) $_value = 1;
...
$tenure));
  ...
  // picture upload
	...
	global $pr_u_adds_pp;
	$pr_ids = array($last_id);
	do_action($pr_u_adds_pp, $pr_ids, $pr_u_adds_pp); // send notification
...
if (!get_option($pr_ntf_opt_names)) {
	$option_names = array();
	$option_names[] = 'Property added (Admin)';
	$option_names[] = 'Property added (User)';
	//$option_names[] = 'Non paid property added (Admin)';
	//$option_names[] = 'Non paid property added (User)';
	$option_names[] = 'Payment confirmation (Admin)';
	$option_names[] = 'Payment confirmation (User)';
...
function pr_ntf_send($pr_ids, $type){
	...
	$pr_ntfs_mapping = array($pr_u_adds_pp=>array(0,1),$pr_u_adds_npp=>array(3,4),$pr_u_adds_pfp=>array(2,3));
...
//	print_r($pr_ids);
	switch($type){
		case $pr_u_adds_pp:
			...
			break;
		case $pr_u_adds_npp:
// do not send notifications for non paid properties
/*
			foreach ($pr_ids as $pr_id){
				// several emails need to be sent
				pr_ntf_send_to($mapping,$pr_id);
...
function pr_ntf_send_to($mapping,$pr_id){
	...
	$admin_mapping = array(0,2);
	$user_mapping = array(1,3);
